...
module.directive('orgTree',[ function () {
    return {
        restrict: 'E',
        replace: true,
        template:"<ul class='treeRoot'><li ng-repeat='row in treeArbre | filter:{visible:true} track by $index' class='treeitem'><span ng-style='row.magin'><i ng-if='row.expand' ng-class=\"row.collapse ? 'fa fa-minus':'fa fa-plus'\" aria-hidden='true' ng-click='expanded(row)'></i><i ng-if='multiSelect' ng-click=\"element_select(row, 'icon')\" ng-class='row.icon' aria-hidden='true'></i><span ng-class='{textColor: row.selected}' class='text' ng-click=\"element_select(row,'org')\">{{row.name}}</span></span></li></ul>",
        scope: {
            orgData: '=',
            multiSelect: '=',
            getAllselect: '&'
        ...
        link: function(scope, element, attrs) {
            ...
            var orgCollapse = [];
            var lesCheckBox = ['fa fa-square-o','fa fa-check-square-o','fa fa-check-square','fa fa-square-o'];
